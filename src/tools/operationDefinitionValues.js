...
const OPERATION_DEFINITION_VALUES = [
  ...
  },
  {
    ...
    description: "Read-only. Batch-read files or directory summaries, load one discovered project/user skill by name, retrieve one execution-scoped outputRef returned by a truncated relai_exec result, or return one exact file as a private MCP resource_link for transfer/download. Ordinary hidden and Git-ignored files can be read when explicitly targeted; snapshot exclusions are not direct-access restrictions. Secret-bearing paths remain blocked. Use asResource:true with exactly one paths entry for whole-file transfer; startLine/endLine and ranges remain text-only. guidanceMode accepts full, compact, or none.",
  ...
  },
  {
    ...
    inputSchema: {"type":"object","properties":{"workspace":{"type":"string"},"action":{"type":"string","enum":["symbol","definition","references","hover","implementation","related","impact","trace","diagnostics","architecture","audit"]},"symbol":{"type":"string","minLength":1,"maxLength":256},"path":{"type":"string","minLength":1,"maxLength":1000},"line":{"type":"number","minimum":1,"maximum":10000000},"column":{"type":"number","minimum":1,"maximum":10000000},"query":{"type":"string","minLength":1,"maxLength":1000},"paths":{"type":"array","items":{"type":"string"},"minItems":1,"maxItems":100},"maxResults":{"type":"number","minimum":1,"maximum":1000},"maxDepth":{"type":"number","minimum":1,"maximum":8},"maxFiles":{"type":"number","minimum":1,"maximum":20000},"refresh":{"type":"boolean","description":"When false, audit may answer from the cached index and refresh in the background. Defaults to true."}},"required":["workspace","action"],"additionalProperties":false},
  ...
  },
  {
    ...
    description: "Run a one-shot development command directly in the configured visible workspace. Mutating or uncertain commands are serialized against other workspace writes; provably workspace-read-only direct commands, including direct ADB invocations that cannot create local files, can overlap safely. Persistent streams and interactive commands such as adb logcat or an interactive adb shell belong in relai_process. Use command for shell syntax, or executable + argv (+ optional input) for shell-free execution that preserves quotes, backticks, dollar signs, and multiline script text exactly. Returns exit status, bounded stdout and stderr, timing, and detected file changes. cwd is workspace-relative. With a durable work_id or native MCP Task, omitted timeoutMs means the command runs until it exits or the task is explicitly cancelled; taskless execution retains a bounded safety timeout. An explicit timeoutMs is always honored. work_id is optional attribution, not execution permission.",
    ...
    handlerName: 'exec',
    behavior: {"audit":"exec","cache":"workspace","longRunning":true,"concurrencyScope":"mutation","taskScope":"optional"},
  },
  {
    ...
    behavior: {"audit":"exec","cache":"workspace","taskScope":"optional"},
  },
  {
  ...
  },
  {
    ...
    inputSchema: {"type":"object","properties":{"workspace":{"type":"string"},"action":{"type":"string","enum":["status","start","tabs","open_tab","close_tab","navigate","snapshot","interact","screenshot","upload","download","stop"]},"sessionId":{"type":"string","pattern":"^browser_[A-Za-z0-9_-]{20,160}$"},"tabId":{"type":"string","pattern":"^tab_[A-Za-z0-9_-]{20,160}$"},"url":{"type":"string","minLength":1,"maxLength":4000},"profile":{"type":"string","enum":["ephemeral","persistent"]},"ignoreHTTPSErrors":{"type":"boolean"},"width":{"type":"number","minimum":320,"maximum":3840},"height":{"type":"number","minimum":240,"maximum":2160},"timeoutMs":{"type":"number","minimum":100,"maximum":30000},"detail":{"type":"string","enum":["semantic","layout"]},"interaction":{"type":"string","enum":["click","fill","press","select","hover","wait"]},"target":{"type":"object","properties":{"by":{"type":"string","enum":["role","text","label","placeholder","testid","css"]},"value":{"type":"string","minLength":1,"maxLength":4000},"name":{"type":"string","maxLength":1000},"exact":{"type":"boolean"},"index":{"type":"number","minimum":0,"maximum":1000}},"required":["by","value"],"additionalProperties":false},"input":{"type":"string","maxLength":1048576},"key":{"type":"string","maxLength":100},"selectValue":{"type":"string","maxLength":10000},"state":{"type":"string","enum":["visible","hidden","attached","detached"]},"fullPage":{"type":"boolean"},"path":{"type":"string","minLength":1,"maxLength":1000}},"required":["workspace","action"],"additionalProperties":false},
    handlerName: 'browser',
    behavior: {"audit":"exec","taskScope":"optional"},
  },
  {
  ...
  {
    name: OP.COMPUTER,
    title: "Host Computer Control",
    description: "Control the user-authorized local desktop with a layered Windows path: UI Automation first, built-in Windows OCR for hybrid fallback, and Midscene pixels only when native semantics cannot perform the action. observe defaults to auto perception (UIA, then OCR when UIA yields no targets); use perception=hybrid when a custom-rendered text control is missing. activate revalidates the target and prefers safe native UIA Invoke/Selection/Toggle/Expand patterns before a current-center click. set_value uses UIA ValuePattern without keyboard focus. wait_for_change and wait_for_stable provide deterministic post-action verification and can be combined with observe inside batch(actions). Coordinates remain display-local and observation-scoped. Computer control, per-app approval, browser read-only routing, terminal/IDE typing restrictions, and the single-session input lock remain enforced.",
    inputSchema: {"type":"object","properties":{"workspace":{"type":"string"},"action":{"type":"string","enum":["status","displays","observe","activate","set_value","screenshot","wait_for_change","wait_for_stable","move","click","double_click","right_click","drag","scroll","type","key","hotkey","batch","stop","approve_app","revoke_app"]},"displayId":{"type":"string","minLength":1,"maxLength":200},"app":{"type":"string","minLength":1,"maxLength":200},"x":{"type":"number","minimum":0,"maximum":100000},"y":{"type":"number","minimum":0,"maximum":100000},"toX":{"type":"number","minimum":0,"maximum":100000},"toY":{"type":"number","minimum":0,"maximum":100000},"direction":{"type":"string","enum":["up","down","left","right"]},"distance":{"type":"number","minimum":1,"maximum":100000},"text":{"type":"string","minLength":1,"maxLength":65536},"value":{"type":"string","maxLength":65536},"key":{"type":"string","minLength":1,"maxLength":100},"keys":{"type":"array","items":{"type":"string","minLength":1,"maxLength":100},"minItems":2,"maxItems":8},"actions":{"type":"array","items":{"type":"object"},"minItems":1,"maxItems":20},"observationId":{"type":"string","minLength":1,"maxLength":100},"previousObservationId":{"type":"string","minLength":1,"maxLength":100},"profile":{"type":"string","enum":["fast","balanced","detail"]},"forceImage":{"type":"boolean"},"timeoutMs":{"type":"number","minimum":100,"maximum":30000},"pollMs":{"type":"number","minimum":50,"maximum":1000},"stableMs":{"type":"number","minimum":100,"maximum":5000},"semanticObservationId":{"type":"string","minLength":1,"maxLength":100},"targetId":{"type":"string","minLength":1,"maxLength":100},"maxElements":{"type":"number","minimum":1,"maximum":300},"perception":{"type":"string","enum":["auto","semantic","hybrid"]}},"required":["workspace","action"],"additionalProperties":false},
    handlerName: 'computer',
    behavior: {"audit":"exec","taskScope":"optional"},
  },
  {
  ...
  },
  {
    ...
    description: "Create an immutable workspace-bound review checkpoint. With work_id it defaults to the task-owned diff; without work_id it checkpoints workspace scope. scope:'task' requires work_id. The stored payload is integrity-checked and can be replayed later by checkpointId without depending on the current working tree.",
    inputSchema: {"type":"object","properties":{"workspace":{"type":"string"},"staged":{"type":"boolean"},"path":{"type":"string"},"redactSensitive":{"type":"boolean"},"scope":{"type":"string","enum":["task","workspace"]},"maxBytes":{"type":"number","minimum":1000,"maximum":5242880}},"required":["workspace"],"additionalProperties":false},
  ...
  },
  {
    ...
    description: "Discard all tracked working-tree and index changes by resetting to HEAD. Set removeUntracked:true to also remove untracked files and directories. The destructive operation is guarded by the native approval flow; no second magic confirmation token is required.",
    inputSchema: {"type":"object","properties":{"workspace":{"type":"string"},"removeUntracked":{"type":"boolean"}},"required":["workspace"],"additionalProperties":false},
  ...
  },
  {
    ...
    description: "Read-only. Return compact durable task/background-operation state by default. Set detail:'full' when repository status, command discovery, runtime/tool-surface metadata, or continuity details are actually needed. Supply work_id for durable task state or operationId to recover a taskless long-running fallback operation.",
  ...
  {
    name: OP.WORK_STOP,
    title: "Stop Running Task Operations",
    description: "Stop one finite running operation by operationId, or all finite running operations in the exact logical task when operationId is omitted. The task remains open and can continue afterward. Managed persistent processes have an independent lifecycle and must be stopped with relai_process action stop.",
    inputSchema: {"type":"object","properties":{"workspace":{"type":"string"},"operationId":{"type":"string","minLength":1,"maxLength":200},"reason":{"type":"string","maxLength":500}},"required":[],"additionalProperties":false},
    handlerName: 'stopTaskOperations',
    behavior: {"audit":"exec"},
    dashboard: {"category":"Workflow"}
  },
  {
