...
const OPERATION_DEFINITION_VALUES = [
  ...
  },
  {
    ...
    description: "Run a one-shot development command directly in the configured visible workspace. Mutating or uncertain commands are serialized against other workspace writes; provably read-only direct commands can overlap safely. Use command for shell syntax, or executable + argv (+ optional input) for shell-free execution that preserves quotes, backticks, dollar signs, and multiline script text exactly. Returns exit status, bounded stdout and stderr, timing, and detected file changes. cwd is workspace-relative. work_id is optional attribution, not execution permission.",
  ...
  },
  {
    ...
    description: "Run workspace validation checks (tests, linters, analyzers, build). Use level quick, standard, or release. Output is bounded to each step's tail where failures appear; pass fullOutput:true for a larger tail. Validation records factual evidence about the repository state. complete:true is an optional convenience that validates and closes an explicitly supplied work_id atomically.",
  ...
  },
  {
    ...
    description: "Read-only. Return configured workspace aliases and tool-surface status. When workspace is provided, also return command configuration, session policy, branch, ahead/behind counts, ownership-split changes, and untracked-file state under workspace.repository. Supply work_id for durable task state or operationId to recover a taskless long-running fallback operation.",
    inputSchema: {"type":"object","properties":{"workspace":{"type":"string"},"operationId":{"type":"string","pattern":"^fallback_[A-Za-z0-9_-]{20,160}$"},"maxBytes":{"type":"number","minimum":1000,"maximum":5242880}},"required":[],"additionalProperties":false},
    handlerName: 'status',
    behavior: {"taskScope":"optional"},
  },
  {
