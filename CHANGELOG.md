...
- **Keep Windows in-app updates visibly active while Rel.AI is replaced.** Silent updater installs now hand off to an installer-owned `Updating Rel.AI MCP…` surface that remains visible after the dashboard closes and explains that Rel.AI will restart automatically. A user-data update marker blocks shortcut launches from entering the normal Electron/Node runtime during the file swap, installer success/failure clears that marker, the replacement `--updated` launch clears any residue, and stale state fails open instead of permanently locking the app.
